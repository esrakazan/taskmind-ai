...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) { setError(data.message || "Kayıt başarısız."); return; }
    ...
    <div className="auth-root">
      <div className="brand-panel">
        ...
        <div className="brand-content">
          ...
          <div className="brand-tagline">
            <h1>Ücretsiz başla,<br />hızla büyü.</h1>
            <p>Dakikalar içinde kurulum yap.<br />AI destekli proje yönetimine geç.</p>
          </div>
          <div className="steps">
            {[{ n: "1", t: "Hesabını oluştur" }, { n: "2", t: "İlk projeyi aç" }, { n: "3", t: "AI'dan plan al" }].map((s) => (
      ...
      <div className="form-panel">
        <div className="form-card">
          <div className="form-header">
            <h2>Hesap oluştur</h2>
            <p>Ücretsiz, kredi kartı gerektirmez</p>
          </div>
          {error && (
          ...
          <form onSubmit={handleSubmit} className="auth-form">
            <div className="field-group">
              ...
              <div className="input-wrapper">
                ...
                <input id="name" type="text" placeholder="Adın Soyadın" value={name} onChange={(e) => setName(e.target.value)} required />
              </div>
            </div>
            <div className="field-group">
            ...
            <div className="field-group">
              ...
              <div className="input-wrapper">
                ...
                <input id="password" type="password" placeholder="En az 8 karakter" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} />
              </div>
            </div>
            <button type="submit" className="submit-btn" disabled={loading}>
              {loading ? <span className="spinner" /> : <>Hesap Oluştur <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M3 8h10M9 4l4 4-4 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg></>}
            </button>
          </form>
          <p className="switch-link">Zaten hesabın var mı? <Link href="/login">Giriş yap</Link></p>
        </div>
      </div>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .step-num { width: 28px; height: 28px; border-radius: 50%; background: rgba(0,229,160,0.15); border: 1px solid rgba(0,229,160,0.3); color: #00E5A0; font-family: 'Syne', sans-serif; font-weight: 700; font-size: 13px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        .step-text { font-size: 14px; color: rgba(255,255,255,0.55); }
